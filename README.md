...
Electron 데스크톱 앱과 웹 클라이언트가 Cloudflare Worker 및 Durable Object를 통해 방 상태와 실시간 연결을 공유합니다. 데스크톱의 게임 문서는 기기에서 직접 열고, 힌트 투표가 통과한 목표 문서에 한해서 서버가 짧은 설명과 분류를 추출합니다. 원문 HTML은 저장하지 않으며, 서버 실행 환경당 최대 512건의 짧은 힌트를 6시간 동안 임시 보관합니다. 나무위키 요청은 4초·2MiB, 위키백과 보조 요청은 전체 6초·JSON 응답당 128KiB, 동시 문서 조회는 8건으로 제한합니다. 실패한 문서는 1분 뒤 재시도할 수 있고, 설명을 임의로 만들어내지 않습니다.
...
아직 달리는 참가자만 힌트 투표에 참여합니다(2명일 때 2표, 4명일 때 3표, 혼자면 1표). v0.4.4부터 **미리 준비한 힌트 카드**를 사용합니다. 1단계는 짧은 설명, 60초 뒤 새 과반수 투표로 여는 2단계는 연관 개념 2~3개입니다. 연관 개념은 실제 연결이나 최단 경로를 보장하지 않습니다. 감점은 없고 완주·포기 시점의 힌트 단계를 결과에 기록합니다.

게임 중 힌트 버튼은 외부 사이트를 요청하지 않습니다. 검토한 카드 64개를 서버에 보관하며, 출발은 기존 15,199개 제목 스냅샷, 랜덤·라운드·새 일일 목표는 카드 지원 목록에서 선택합니다. 직접 지정 목표는 제한하지 않지만 카드가 없으면 대기실부터 힌트 미지원 안내를 보여줍니다. 전체 나무위키 문서를 지원하는 것은 아닙니다.

카드 출처는 한국어 위키백과 기여자이며 짧은 설명의 발음·번역 괄호 등을 일부 생략·정리했습니다. CC BY-SA 4.0 고지를 유지합니다. 전체 카드와 미공개 단계는 클라이언트에 전송하지 않습니다. 기존 방의 경로·이미 공개된 힌트·결과 기록은 유지하고, 새 라운드는 준비된 카드로 시작합니다. 웹 새로고침 또는 프로그램 업데이트가 필요합니다.

카드 확장과 검토 절차는 [서버 힌트 카드 안내](server/data/README.md), 출처 조건은 [제3자 권리 고지](THIRD_PARTY_NOTICES.md)를 참고하세요. 게임 서버는 `https://namu-race-online.namu-race-online.workers.dev`입니다. 운영 검증은 `NAMU_RACE_TEST_URL`, `NAMU_RACE_HINT_TITLE`, 두 단계 검증은 `NAMU_RACE_FULL_HINT_SMOKE=1`을 지정하고 `node test/smoke-hints-online.mjs`를 실행합니다.
...
Windows 설치 파일은 `release/NamuRace-online-0.4.4-beta.1-setup.exe`입니다. macOS DMG는 포함된 GitHub Actions 워크플로에서 Intel Mac과 Apple Silicon Mac을 모두 지원하는 universal 앱으로 빌드합니다.
