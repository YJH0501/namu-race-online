...
힌트의 알려진 제한: 2026-09-06 운영 환경에서 나무위키 원본이 서버 요청을 403으로 거부하고 웹 문서 서비스가 이를 502로 반환하는 문제를 확인했습니다. v0.4.3은 원본 요청을 반복하거나 차단을 우회하지 않고, 실패 시 한국어 위키백과의 공식 요약·분류 API로 보완합니다. 제목이 정확히 일치하는 일반 문서만 허용하며 괄호의 구분자·하위 문서·단어 간 공백을 없애거나 유사 검색으로 맞추지 않습니다. 동음이의·제목 불일치·없는 문서·불완전 응답은 제외합니다. 위키백과에 대응 항목이 없으면 여전히 힌트를 제공하지 못할 수 있습니다. 제목 일치 검증은 두 위키의 모든 서술이나 편집 판단이 동일함을 보장하지 않습니다.

보조 힌트는 위키백과 기여자, 실제 문서 주소, CC BY-SA 4.0과 라이선스 주소, 발췌·일부 생략 사실을 표시합니다. 2단계 설명과 출처를 방 상태에 짧게 보관해 서버가 잠들었다 재개되어도 같은 자료를 공개합니다. 게임 이동 링크는 추가하지 않습니다. 새 출처 표시는 웹 새로고침 또는 데스크톱 v0.4.3 이상에서 지원합니다. 운영 검증 시 `NAMU_RACE_HINT_TITLE`로 문서를, `NAMU_RACE_EXPECT_HINT_SOURCE=wikipedia`로 기대 출처를 지정할 수 있습니다. 로컬 요청의 성공은 운영 서버 경로 검증을 대체하지 않습니다.
