...
힌트의 알려진 제한: 2026-09-06 운영 환경에서 게임 서버 → 웹 문서 서비스 → 나무위키 요청이 원본의 HTTP 403으로 거부되는 문제를 확인했습니다. 웹 문서 서비스는 이를 502로 반환합니다. 로컬 요청이나 단일 문서의 성공은 운영 힌트의 신뢰성을 입증하지 않습니다. 원인 진단만 완료되었으며 안정적인 힌트 제공 문제는 아직 해결되지 않았습니다. `NAMU_RACE_HINT_TITLE`을 지정해 운영 검증 문서를 바꿀 수 있습니다. 접근 거부를 우회하거나 없는 설명을 생성하지 않습니다.
